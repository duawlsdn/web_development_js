...
### 화살표 함수
: 화살표로 값을 정의하는 함수로, 이 때는 function을 선언하지 않고, 지금까지 사용했던 함수와도 기능이 좀 다릅니다.
```
...
<head>
  ...
  <script>
    // 여태까지는 여기에 파일 경로를 작성, 
    // 단순한 자바스크립트 코드라인의 겨웅는
    // script 태그 내에 곧장 작성해도 괜찮습니다.
    alert("Hello, World!")  // 알림창 느낌!
...
```
#### document.write()
: 입력받은 문자열을 바디<body> 태그 안에 그대로 작성해주는 메서드로, 직관적으로 바디<body> 태그안에 넣을 수 있기는 한데, 추후에 배우게 될 DOM 요소를 다루는 방법에 비해 활용도가 낮으므로 특수한 경우가 아니면 자주 사용되지 않음
...
```
로 적용돼있고, 그 쪽 라인에 user agent stylesheet 라고 설명되어있습니다.

: 사용자가 지정한 스타일이 아니라 브라우저의 기본 스타일을 의미함.

개발자 도구 - elements - style 부분을 주목하시면, 활성화 되어있는 부분에 대한 style 편집 및 확인 가능한데, 
```
element.style {
  // 여기에 css 편집
}
```
하시면 됩니다.
그리고 vs code에서와 달리 체그박스 해제를 통해서 바로 대조가 가능하여 굳이 코드를 썼다 지웠다 할 필요가 없음

#### 이름을 입력 받고, 출력하기

: 이름을 입력받기 위해서 사용할 함수 -> prompt() // alert 과 유사하지만 입력받는 창이 적용됨.

#### 큰따옴표 vs 작은따옴표
개발자도구에 prompt() 에 넣은 문자열을 강제로 ""를 썼는데, return 값도 "" 로 쌓여있음을 확인할 수 있음. 근데 JS의 경우 "" 와 '' 의 구분이 없습니다.

그럼 왜 굳이 큰따옴표와 작은따옴표를 구분하냐면, Java의 경우 문자열을 ""로 쓰기 떄문에, 문자열 내부에 ""를 넣는 것이 굉장히 번거롭습니다. \" 을 써야 하지요.

그런데 Js / Python의 경우 ""와 ''를 둘 다 쓸 수 있기 때문에 '안녕하세요., "안근수"입니다.'와 같은 방식으로 str을 표기 할 수 있습니다.
...
그래서 지금 이후 수업부터는 JS 작성시에 스크립트 내에서는 ''를 쓰도록 함
...
```
로 작성했을 때
alert 으로 뜬 값이 45가 나왔습니다.

Why?

: prompt의 리턴값은 string 으로 되어있어서
...
이는 변수 선언을 하면서 자료형을 명시하는  Java와, 적당히 넘어가는 JS간의 차이 때문입니다.
...
- GPS API : 현재 위치를 요청하면 위도 / 경도로 구성된 좌표로 return

- alert() / prompt() 등 : 브라우저가 제공하는 API로 특정한 결과값을 요구함. 이상의 경우 prompt()의 return 값은 str이기 떄문에 수학 연산 등의 과정을 위해서 추가적인 코드를 작성할 필요가 있음을 공식 문서 등에서 확인 가능
...
```
// for문
let totalNumber = Number(prompt('몇번의 숫자를 더하시겠습니까?'));
let sum = 0;
...
: 실행 할 때마다 매번 다른 값을 return 해주는 함수
-> 0이상 1미만의 number 값을 return